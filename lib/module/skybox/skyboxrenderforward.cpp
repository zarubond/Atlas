...
#include "skyboxrenderforward.h"

SkyBoxRenderForward::SkyBoxRenderForward()
{
    x=0;
    y=0;
    z=0;
...
void SkyBoxRenderForward::renderPrev(WorldGraphics *world, const Environment *envi, const Camera &camera, unsigned int elapsed)
{
    shader.bind();
...
bool SkyBoxRenderForward::loadShader()
{
    if(shader.loadFile(":lib/shader/SkyboxForward.vsh",":lib/shader/SkyboxForward.fsh")==false)
        return false;
...
void SkyBoxRenderForward::renderSetup(SkyBoxGraphics *skybox)
{
    skybox->vao.create();
    skybox->vao.bind();
