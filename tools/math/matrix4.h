...
public:
    ...
    /**
     *  @brief Prints the matrix on standard output.
     */
    void print () const;
...
/*
    template <typename SType>
    Vertex4<Type> operator * (const Vertex4<SType> & vex) const;*/
    /**
   ...
   // friend std::ostream & operator << <> (std::ostream & out, const Matrix4<Type> & mat);
    ...
    Matrix4<Type> & translate(const Vertex3<Type> & translate);
    /**
     ...
     * @param up
     */
    void lookAt(const Vertex3<Type> &eye, const Vertex3<Type> &center, const Vertex3<Type> &up);
    /**
    ...
    friend Vertex4<OType> operator *(const Matrix4<OType> & matrix,const Vertex4<OType> & vertex);
protected:
    Type matrix[16];
...
template <typename OType>
Vertex4<OType> operator *(const Matrix4<OType> & matrix,const Vertex4<OType> & vertex)
{
    return Vertex4<OType>(matrix[0]*vertex[0] + matrix[1]*vertex[1] + matrix[2]*vertex[2]  + matrix[3]*vertex[3],
