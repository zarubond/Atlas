...
Vertex3<float> Quaternion::operator *(const Vertex3<float> &vec) const
{
    Vertex3<float> vn(vec);
    vn.normalize();
    ...
    return (Vertex3<float>(resQuat.x, resQuat.y, resQuat.z));
}
...
void Quaternion::lookRotation(const Vertex3<float>& lookAt, Vertex3<float> up)
{
    Vertex3<float> forward = lookAt;
    forward.normalize();
    Vertex3<float> right = up*forward;
    right.normalize();
    up =forward* right;
...
void Quaternion::rotate(const Vertex3<float> &angle)
{
    rotate(angle[0],angle[1],angle[2]);
}

Vertex3<float> Quaternion::getAngle() const
{
    Vertex3f ang;
    ang[0]=atan((2*(x*y+z*w))/(1-2*(y*y+z*z)));
    ang[1]=asin(2*(x*z-w*y));
    ang[2]=atan((2*(x*z+y*z))/(1-2*(z*z+w*w)));
    return ang;
}

void Quaternion::direction(const Vertex3<float> &dir)
{
